/*
     ...
     jsNGContent - Library of code for handling the content of a Norton Guide.
...
function NGMenu( ng ) {
    ...
    const title = ng.clean( ng.readStringZ( MAX_PROMPT_LEN ) );

    // Now load each of the prompts.
    for ( let i = 0; i < promptCount; i++ ) {
        prompts.push( ng.clean( ng.readStringZ( MAX_PROMPT_LEN ) ) );
    }
...
function NGEntry( ng ) {
    ...
    this.next            = () => next;
    this.hasParent       = () => this.parent()       != -1;
    this.hasParentLine   = () => this.parentLine()   != -1;
    this.hasParentMenu   = () => this.parentMenu()   != -1;
    this.hasParentPrompt = () => this.parentPrompt() != -1;
    this.hasPrevious     = () => this.previous()     != -1;
    this.hasNext         = () => this.next()         != -1;
    this.lines           = () => lines;
    this.offsets         = () => offsets;
